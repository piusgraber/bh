<script lang="ts">
    ...
    async function processCSVFile(file: File) {
        ...
        try {
            ...
            const response = await fetch('/api/xfact', {
                method: 'POST',
                body: formData
            });
            ...
            if (result.success) {
                csvData = result.data;
                console.log('Parsed xfact data:', csvData);
            } else {
...
<div class="import-container">
    <h1>Xfact CSV Import</h1>
    <p class="description">
        Importieren Sie CSV-Dateien im Xfact-Format. Die Datei sollte die Spalten 
        Datum, R-Dat, Auft. Nr, Rech. Nr, Adresse, Bezeichnung, Betrag, Art enthalten.
    </p>
                        ...
                        <tbody>
                            {#each csvData.slice(0, 10) as row}
                                <tr>
                                    ...
                                    <td class="betrag">{formatCurrency(row.betrag)}</td>
                                    <td>{row.soll}</td>
                                    <td>{row.haben}</td>
